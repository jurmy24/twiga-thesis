...
class DataSearch:
    ...
    def search(self, size:int, knn_args: dict[str, Any]=None, query_args:dict[str, Any]=None, rank_args:dict[str, Any]=None) -> dict:
        try:
            if knn_args and query_args and rank_args:
                res = self.es.search(index="twiga_documents", size=size, query=query_args, knn=knn_args, rank=rank_args)
            elif knn_args:
                res = self.es.search(index="twiga_documents", size=size, knn=knn_args)
            elif query_args:
                res = self.es.search(index="twiga_documents", size=size, query=query_args)
            else:
