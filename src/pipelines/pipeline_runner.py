...
from src.utils import (
    get_embedding,
    load_json_to_evalquery,
    load_json_to_pipelinedata,
    save_objects_as_json,
)
...
def process_queries(file_path: str):
    ...
    for eval_query in tqdm(queries, desc="Rewriting queries..."):
        ...
        rewritten_query = query_rewriter(
            original_query, llm="llama3-8b-8192"
        )  # would be better if this could be done as a batch but I guess not...
        ...
        rwq: RewrittenQuery = RewrittenQuery(
            rewritten_query_str=rewritten_query, embedding=rewritten_embedding
        )
...
def process_rewritten_queries(file_path: str) -> List[PipelineData]:
    ...
    for item in tqdm(pipe_data, desc="Retrieving documents"):
        ...
        pip_data: PipelineData = PipelineData(
            query=item.query, retrieved_docs=retrieved_docs
        )
...
def run_data_through_generator(
    pipe_data: List[PipelineData],
    model: Literal["gpt-3.5-turbo-0125", "gpt-4-turbo-2024-04-09", "llama3-70b-8192"],
    ablation_params: Dict[str, bool] | None = None,
    verbose: bool = False,
) -> List[PipelineData]:
    results: List[PipelineData] = []
    ...
    if ablation_params is not None:
        (
            ...
            ablation_params.values()
        )  # the first three don't have an impact on the generation phase
    else:
        (
            no_rewriter,
            no_sparse_retriever,
    ...
    for item in tqdm(pipe_data, desc="Running through generator..."):

        retrieved_docs = item.retrieved_docs

        retrieved_content = [
            doc for doc in retrieved_docs if doc.source.metadata.doc_type == "Content"
        ]
        ...
        system_prompt = (
            PIPELINE_QUESTION_GENERATOR_PROMPT.format()
            if not no_one_shot
            else PIPELINE_QUESTION_GENERATOR_PROMPT_ZERO_SHOT.format()
        )
        user_prompt = PIPELINE_QUESTION_GENERATOR_USER_PROMPT.format(
            query=item.query.query, context_str=context
        )

        res = pipeline_generator(system_prompt, user_prompt, model, verbose=verbose)

        res_proper: ResponseSchema = ResponseSchema(
            text=res, embedding=get_embedding(res, embedding_model)
        )

        pip_data: PipelineData = PipelineData(
            query=item.query, retrieved_docs=retrieved_docs, response=res_proper
        )
...
def process_pipeline_item(
    ...
    if not no_sample_questions:
        retrieved_exercise = [
            doc for doc in retrieved_docs if doc.source.metadata.doc_type == "Exercise"
        ]
...
def run_data_through_generator_threaded(
    pipe_data: List[PipelineData],
    model: Literal["gpt-3.5-turbo-0125", "gpt-4-turbo-2024-04-09", "llama3-70b-8192"],
    ablation_params: Dict[str, bool] | None = None,
    verbose: bool = False,
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in tqdm(
            ...
            total=len(futures),
            desc="Running through generator...",
        ):
...
if __name__ == "__main__":
    ...
    input_file = os.path.join(
        DATA_DIR, "datasets", "test-prompts-rewritten-retrieved.json"
    )
    ...
    res = run_data_through_generator(
        incomplete_pipeline_data, "llama3-70b-8192", verbose=False
    )
