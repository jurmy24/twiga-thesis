...
def compute_ragas_results(dataset: Dataset, verbose: bool=False) -> pd.DataFrame:
    result = evaluate(
        dataset,
        metrics=[
            faithfulness,
...
if __name__ == "__main__":
    ...
    csv_file_ragas = os.path.join(DATA_DIR, "results", "pipeline-5-ragas-analysis.csv")

    pipeline_data = extract_eval_data(data_file)[250:300]
    ...
    df_ragas = compute_ragas_results(dataset)
    ...
    merged_df.to_csv(csv_file_ragas, mode='a', header=False) # set index=False
    ...
    # DATA_DIR = os.getenv("DATA_DIR_PATH")
    # results_file = os.path.join(DATA_DIR, "results", "results.txt")
    # df = pd.read_csv(os.path.join(DATA_DIR, "results", "pipeline-5-ragas-analysis.csv"))

    # # Columns to calculate the mean
    # columns_to_average = ['faithfulness', 'answer_relevancy', 'context_relevancy', 'reverse_context_relevancy']  # Specify the columns you want to average

    # """COMPUTE THIS FROM THE CSV FILE INSTEAD"""
    # # Compute the mean for specified columns
    ...
    # append_to_file(results_file, f"Pipeline (5) RAGAS Groundedness: {average_gr}, Answer Relevancy: {average_ar}, Context Relevancy: {average_cr}, Reverse Context Relevancy: {average_rcr}")